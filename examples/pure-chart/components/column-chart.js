...
export default class ColumnChart extends Component {
  constructor(props) {
    super(props)
    const defaultGap = this.props.defaultColumnWidth + this.props.defaultColumnMargin
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.defaultColumnWidth !== this.props.defaultColumnWidth || nextProps.defaultColumnMargin !== this.props.defaultColumnMargin ) {
      const defaultGap = nextProps.defaultColumnWidth + nextProps.defaultColumnMargin
      this.setState({
        gap: defaultGap,
      })
    }
  ...
  renderColumns(fadeAnim) {
    const seriesArray = this.state.sortedData
    const seriesCount = seriesArray.length
    const renderColumns = []
    if (seriesCount > 0) {
      const standardSeries = seriesArray[0]
      const dataCount = standardSeries.data.length
      ...
      <Animated.View
        style={[
          styles.chartView,
          {
            ...
            marginBottom:
              this.props.minValue && this.state.guideArray && this.state.guideArray.length > 0
  ...
  drawTooltip(selectedIndex) {
    if (typeof selectedIndex === 'number' && selectedIndex >= 0) {
      const standardSeries = this.state.sortedData[0]
      if (!standardSeries) {
        console.warn('standardSeries is null')
        return null
      }

      const seriesCount = this.state.sortedData.length
      ...
      const left = standardSeries.data[selectedIndex].gap + plusGap
      const tooltipRenders = []
      for (let i = 0; i < this.state.sortedData.length; i++) {
        const series = this.state.sortedData[i]
      ...
      return (
        <View style={[styles.tooltipWrapper, { left }]}>
